...
Scene * WinScene::create()
{
	auto scene=new WinScene();
	if(!scene->init())
	{
		delete scene;
		return nullptr;
...
bool WinScene::init()
{
	if(!Scene::init())
		return false;
	//background music:
	CocosDenshion::SimpleAudioEngine::getInstance()->preloadBackgroundMusic(WinMusicMp3.c_str());
	CocosDenshion::SimpleAudioEngine::getInstance()->playBackgroundMusic(WinMusicMp3.c_str(),true);
	//background:
	Size visibleSize = Director::getInstance()->getVisibleSize();
    Vec2 origin = Director::getInstance()->getVisibleOrigin();
	auto background=Sprite::create(WinPng);
	...
	//创建文字标签:
	auto label=Label::create("You Win!!~~~","宋体",40);
	...
	auto menuitem_1=MenuItemLabel::create(label_1,CC_CALLBACK_1(WinScene::menuCloseCallback,this));
	...
	auto menuitem_2=MenuItemLabel::create(label_2,CC_CALLBACK_1(WinScene::menuCloseCallback,this));
